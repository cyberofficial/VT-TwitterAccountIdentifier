...
window.setInterval(function ()
{
	...
	try
	{
		for (const div of document.querySelectorAll('div[data-testid="UserCell"], div[data-testid="TypeaheadUser"], span[data-testid="socialContext"]'))
		{
	...
	try
	{
		try
		{
			var mainprofile_innerhtml = document.querySelector("main").querySelector('div[aria-label="Home timeline"]').lastElementChild
				.firstElementChild.querySelector('div').firstElementChild.lastElementChild.querySelector('div[data-testid="UserName"] > div > div')
				.lastElementChild.querySelector('span').innerHTML;
		}
		catch (ex)
		{
			if (debugmode)
		...
		if (handle != docurl)
		{
			...
			document.querySelector("main").querySelector('div[aria-label="Home timeline"]').lastElementChild.firstElementChild.querySelector('div')
				.firstElementChild.lastElementChild.querySelector('div[data-testid="UserName"] > div > div').lastElementChild.querySelector('span')
				.innerHTML = mainprofile_innerhtml.replace(handle_or, docurl_or);
		}
		...
		try
		{
			if (!mainprofile_innerhtml.includes(`aria-label="vtchecked main profile"`))
			{
				...
				if (vtchecked_users.includes(docurl) && !mainprofile_innerhtml.includes(`aria-label="vtchecked main profile"`))
				{
					...
					document.querySelector("main").querySelector('div[aria-label="Home timeline"]').lastElementChild.firstElementChild.querySelector(
							'div').firstElementChild.lastElementChild.querySelector('div[data-testid="UserName"] > div > div').lastElementChild
						.querySelector('span').innerHTML = mainprofile_innerhtml + ` ` + icon2.replace("replaceme", "main profile");
				}
			}
			else
			{
				...
				if (!vtchecked_users.includes(docurl))
				{
					...
					document.querySelector("main").querySelector('div[aria-label="Home timeline"]').lastElementChild.firstElementChild.querySelector(
							'div').firstElementChild.lastElementChild.querySelector('div[data-testid="UserName"] > div > div').lastElementChild
						.querySelector('span').querySelector(`svg`).outerHTML = "";
				}
				else
				{
					if (!mainprofile_innerhtml.includes(`aria-label="vtchecked main profile"`))
